...
class MQTTClient:
    def __init__(A, client_id, server, port=0, user=None, password=None, keepalive=0, ssl=False, ssl_params=None,
    ...
    def connect(A, clean_session=True):
        ...
        if A.lw_topic: D += 2 + len(A.lw_topic) + 2 + len(A.lw_msg);B[7] |= 4 | (A.lw_qos & 1) << 3 | (
                A.lw_qos & 2) << 3;B[7] |= A.lw_retain << 5
    ...
    def disconnect(A):
        A._write(b'\xe0\x00')
        A.poller_r.unregister(A.sock)
        A.poller_w.unregister(
            A.sock)
        A.poller_r = None
        A.poller_w = None
        A.sock.close()
    ...
    def subscribe(A, topic, qos=0):
        ...
        A.rcv_pids[C] = ticks_add(
            ticks_ms(), A.message_timeout * 1000)
        return C
