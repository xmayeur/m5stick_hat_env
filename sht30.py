...
class SHT30():
    ...
    * https://www.sensirion.com/fileadmin/user_upload/customers/sensirion/Dokumente/11_Sample_Codes_Software/Humidity_Sensors/Sensirion_Humidity_Sensors_SHT3x_Sample_Code_V2.pdf
    ...
    def __init__(self, scl_pin=26, sda_pin=0, delta_temp=-4, delta_hum=0, i2c_address=DEFAULT_I2C_ADDRESS):
        self.i2c = I2C(scl=Pin(scl_pin), sda=Pin(sda_pin))
        self.i2c_addr = i2c_address
        self.set_delta(delta_temp, delta_hum)
        time.sleep_ms(50)

    def init(self, scl_pin=26, sda_pin=0):
        """
...
ntp = ntptime.client(host='be.pool.ntp.org', timezone=2)
rtc.setTime((ntp.year()), (ntp.month()), (ntp.day()), (ntp.hour()), (ntp.minute()), (ntp.second()))
lcd.clear()
lcd.fill(0x000000)
while True:
    temp, hum = sensor.measure()
    lcd.print((ntp.formatDatetime('dd/mm/YY', 'hh:mm:ss')), 1, 1, 0xffffff)
